...
export default function AssistantWidget({ currentPage, contextProjectId, onNavigate, onOpenProject }) {
  ...
  const send = async (questionRaw) => {
    ...
    try {
      const page = contextProjectId ? `project:${contextProjectId}` : currentPage
      const res = await askAssistant(question, page)
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-[70] flex h-13 w-13 items-center justify-center rounded-full bg-[#031632] text-xl text-white shadow-soft transition hover:scale-105 hover:bg-[#0a2545] dark:bg-blue-600 dark:hover:bg-blue-500"
      ...
      {/* Panel */}
      {open && (
        <div className="fixed bottom-20 right-5 z-[70] flex h-[520px] max-h-[calc(100vh-6rem)] w-[380px] max-w-[calc(100vw-2.5rem)] flex-col overflow-hidden rounded-2xl border border-[#dcdde4] bg-white shadow-soft-lg dark:border-[#3f4657] dark:bg-[#111827]">
          {/* Header */}
          <div className="flex flex-none items-center gap-2.5 border-b border-[#dcdde4] bg-[#f9f9ff] px-5 py-4 dark:border-[#3f4657] dark:bg-[#0b1220]">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[0.6875rem] text-[#44474d] dark:text-[#9ca3af]">
                {user ? `For: ${user.name.split(" ")[0]} (${user.role.replace("_", " ")})` : "Public help — sign in for more"}
              </p>
            </div>
          </div>
          ...
          <div ref={scrollRef} className="min-h-0 flex-1 space-y-3.5 overflow-y-auto px-4 py-4">
            {messages.length === 0 && (
              <div className="rounded-xl bg-[#f0f3ff] p-4 text-[0.8125rem] leading-snug text-[#031632] dark:bg-[#1f2937] dark:text-[#f3f4f6]">
            ...
            {messages.length === 0 && contextHelp && (
              <button
                ...
                className="w-full rounded-xl border border-[#dcdde4] bg-white p-3 text-left transition hover:bg-[#f0f3ff] dark:border-[#3f4657] dark:bg-[#111827] dark:hover:bg-[#1f2937]"
              >
            ...
            {messages.map((m, i) =>
              m.role === "user" ? (
                <div key={i} className="flex justify-end">
                  <p className="max-w-[85%] rounded-2xl rounded-br-md bg-[#031632] px-3.5 py-2.5 text-[0.8125rem] text-white dark:bg-blue-600">
                    {m.text}
                  </p>
                </div>
              ) : (
                <div key={i} className="flex justify-start">
                  <div className="max-w-[92%] rounded-2xl rounded-bl-md border border-[#dcdde4] bg-[#f9f9ff] px-3.5 py-2.5 text-[#151c27] dark:border-[#3f4657] dark:bg-[#1f2937] dark:text-gray-100">
                    <AnswerBody text={m.text} />
                    {m.actions?.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1.5 border-t border-[#dcdde4] pt-2 dark:border-[#3f4657]">
                        {m.actions.map((a, j) => (
                          <button
                            key={j}
                            onClick={() => runAction(a)}
            ...
            {busy && (
              <div className="flex justify-start">
                <div className="flex items-center gap-1.5 rounded-2xl border border-[#dcdde4] bg-[#f9f9ff] px-3 py-2.5 dark:border-[#3f4657] dark:bg-[#1f2937]">
          ...
          {suggestions.length > 0 && !busy && (
            <div className="flex flex-none flex-wrap gap-1.5 border-t border-[#dcdde4] px-3 pt-2.5 dark:border-[#3f4657]">
              {suggestions.slice(0, 5).map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="rounded-full border border-[#dcdde4] bg-white px-2.5 py-1 text-[0.6875rem] font-semibold text-[#031632] transition hover:bg-[#f0f3ff] dark:border-[#3f4657] dark:bg-[#111827] dark:text-[#f3f4f6] dark:hover:bg-[#1f2937]"
                >
                  {s}
                </button>
              ))}
          ...
          <div className="flex flex-none items-center gap-2 p-3">
            <input
              ...
              maxLength={500}
              className="min-w-0 flex-1 rounded-full border border-[#dcdde4] bg-white px-3.5 py-2 text-[0.8125rem] outline-none focus:border-[#031632] dark:border-[#3f4657] dark:bg-[#1f2937] dark:text-gray-100 dark:focus:border-blue-500"
            />
            <button
              onClick={() => send()}
